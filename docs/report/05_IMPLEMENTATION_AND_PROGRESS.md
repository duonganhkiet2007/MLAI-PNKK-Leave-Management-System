...
| Rule Engine | `PARTIAL` | `rule_engine.py`, `taxonomy.py`, `domain.py` | Deterministic staged evaluation for annual, medical, special paid, statutory/other unpaid, calendar/overlap/balance/authority. Sick medical is auto-approved at 1 working day with valid/verified proof and routes to Direct Manager at ≥2 working days. | Implement or formally exclude maternity/work accident; resolve unsupported-type outcome mismatch; expand policy coverage. |
...
| Anti-abuse | `PARTIAL` | `storage.load_context`, `rule_engine.py` | Flags prior approved annual days plus the current request in the same calendar month when the cumulative total exceeds the auto threshold, then routes Direct Manager. | Add the policy-required audit detail: related request IDs, period, total and final approver. |
